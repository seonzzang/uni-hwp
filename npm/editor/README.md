...
메뉴, 툴바, 서식, 표 편집 — apps/studio의 모든 기능을 그대로 사용할 수 있습니다.
...
| 옵션 | 기본값 | 설명 |
|------|--------|------|
| `studioUrl` | `https://edwardkim.github.io/rhwp/` | apps/studio URL |
| `width` | `'100%'` | iframe 너비 |
...
셀프 호스팅 환경에서는 apps/studio 빌드에 포함된 `web/fonts/` 폴더의 오픈소스 폰트가 자동으로 사용됩니다. 추가 폰트를 원하면 `web/fonts/`에 woff2 파일을 추가하고 CSS `@font-face`를 등록하면 됩니다.
...
이 패키지는 iframe을 통해 apps/studio를 임베드하며, 내부적으로 `@rhwp/core` WASM 엔진을 사용합니다.
