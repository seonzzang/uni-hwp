...
Uni-HWP는 RHWP 기준선 번호를 따라 `8.6.x` 계열로 관리합니다.

- RHWP `v0.8.6`은 Uni-HWP `8.6.0`에 대응합니다.
- 같은 RHWP 기준선 아래의 후속 변경은 `8.6.1`, `8.6.2`처럼 patch를 올립니다.
...
| --- | --- | --- | --- |
| 8.6.0 | 2026-09-14 | `edwardkim/rhwp@v0.8.6` | RHWP 런타임 엔진을 `v0.8.6`으로 갱신하고, 엔진 업데이트·검증·복원 및 제품 버전 자동 정렬을 반영한 현재 릴리즈입니다.
| 8.4.0 | 2026-08-15 | `edwardkim/rhwp@v0.8.4` | RHWP 엔진 코어를 `v0.7.8`에서 `v0.8.4`로 상향하고, 암호 문서/저장 호환성, 렌더·레이아웃 정합, CLI·배포 채널 정비를 반영한 릴리즈입니다.
...
현재 RHWP 업스트림 기준선은 `edwardkim/rhwp@v0.8.6`입니다.
즉, Uni-HWP의 현재 릴리즈는 `8.6.0`이며, RHWP 런타임 엔진을 `v0.8.6`으로 올린 상태를 반영합니다. 이후 버전은 `8.6.x` 계열로 누적됩니다.
