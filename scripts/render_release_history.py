...
def main() -> int:
    ...
    for release in releases:
        ...
        rows.append(
            "<tr>"
            f'<td class="history-date"><div>{html.escape(date)}</div><div class="history-version">v{html.escape(version)}</div></td>'
    ...
    sys.stdout.write("\n".join(rows) if rows else '<tr><td class="history-empty" colspan="3">아직 공개된 릴리즈가 없습니다.</td></tr>')
    return 0
