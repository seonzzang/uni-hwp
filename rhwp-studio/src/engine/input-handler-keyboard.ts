...
export async function writeImageToClipboard(
  wasm: UniHwpEngine, sec: number, ppi: number, ci: number,
  ...
  if (this.cursor.isInPictureObjectSelection()) {
    ...
    if (e.key === 'Delete' || e.key === 'Backspace') {
      ...
      if (ref) {
        ...
        this.pictureObjectRenderer?.clear();
        this.eventBus.emit('picture-object-selection-changed', false);
        this.executeOperation({ kind: 'snapshot', operationType: 'deleteObject', operation: (wasm: UniHwpEngine) => {
          if (ref.type === 'image') {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'x') {
      ...
      if (ref) {
        ...
        this.pictureObjectRenderer?.clear();
        this.eventBus.emit('picture-object-selection-changed', false);
        this.executeOperation({ kind: 'snapshot', operationType: 'cutObject', operation: (wasm: UniHwpEngine) => {
          if (ref.type === 'image') {
            wasm.deletePictureControl(ref.sec, ref.ppi, ref.ci);
          } else {
            wasm.deleteShapeControl(ref.sec, ref.ppi, ref.ci);
  ...
  if (this.cursor.isInTableObjectSelection()) {
    ...
    if (e.key === 'Delete' || e.key === 'Backspace') {
      ...
      if (ref) {
        if (ref.cellPath && ref.cellPath.length > 1) {
          ...
          this.cursor.moveOutOfSelectedTable();
          this.eventBus.emit('table-object-selection-changed', false);
          this.executeOperation({ kind: 'snapshot', operationType: 'deleteTable', operation: (wasm: UniHwpEngine) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'x') {
      ...
      if (ref && !(ref.cellPath && ref.cellPath.length > 1)) {
        ...
        this.cursor.moveOutOfSelectedTable();
        this.eventBus.emit('table-object-selection-changed', false);
        this.executeOperation({ kind: 'snapshot', operationType: 'cutTable', operation: (wasm: UniHwpEngine) => {
          wasm.deleteTableControl(ref.sec, ref.ppi, ref.ci);
          return this.cursor.getPosition();
        }});
        this.checkTransparentBordersTransition();
...
export function onPaste(this: any, e: ClipboardEvent): void {
  ...
  if (this.wasm.hasInternalClipboard()) {
    ...
    if (this.wasm.clipboardHasControl() && pos.parentParaIndex === undefined) {
      this.executeOperation({ kind: 'snapshot', operationType: 'pasteControl', operation: (wasm: UniHwpEngine) => {
    ...
    this.executeOperation({ kind: 'snapshot', operationType: 'pasteInternal', operation: (wasm: UniHwpEngine) => {
      if (hasSelection) this.deleteSelection();
      const p = this.cursor.getPosition();
      let result: string;
  ...
  if (html) {
    this.executeOperation({ kind: 'snapshot', operationType: 'pasteHtml', operation: (wasm: UniHwpEngine) => {
      if (hasSelection) this.deleteSelection();
      const p = this.cursor.getPosition();
      let result: string;
      if (p.parentParaIndex !== undefined) {
...
async function pasteImageFile(this: any, file: File, hasSelection: boolean): Promise<void> {
  try {
    ...
    // 스냅샷으로 삽입 (Undo 지원)
    this.executeOperation({ kind: 'snapshot', operationType: 'pasteImage', operation: (wasm: UniHwpEngine) => {
      if (hasSelection) this.deleteSelection();
      const p = this.cursor.getPosition();
